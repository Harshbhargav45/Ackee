...
impl Calculator {
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64>{
        if let Some(operation) = self.history.get(operation_index) {
            let op_to_repeat = operation.clone();
            
            match op_to_repeat.operation_type {
                OperationType::Addition => self.addition(op_to_repeat.first_num, op_to_repeat.second_num),
                OperationType::Subtraction => self.subtraction(op_to_repeat.first_num, op_to_repeat.second_num),
                OperationType::Multiplication => self.multiplication(op_to_repeat.first_num, op_to_repeat.second_num),
            }
        } else {
            None
        }
