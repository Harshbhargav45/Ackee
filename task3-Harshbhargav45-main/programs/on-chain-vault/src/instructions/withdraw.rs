...
pub fn _withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
    if ctx.accounts.vault.to_account_info().lamports() < amount {
    ...
    **ctx.accounts.vault_authority.to_account_info().try_borrow_mut_lamports()? = ctx.accounts.vault_authority.to_account_info().lamports()
        .checked_add(amount)
        .ok_or(VaultError::Overflow)?;
